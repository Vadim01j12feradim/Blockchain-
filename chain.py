...
class TestBlockChain(unittest.TestCase):
    def writedoc(self, info):
        f = open ('data.txt','r')
        mensaje = f.read()
        f.close()
        f = open ('data.txt','w')
        f.write(mensaje+info)
        f.close()
    ...
    def test_add_block(self):
        ...
        json1 = json.dumps(self.blockchain.repr_json(), cls=ComplexEncoder, sort_keys=True, indent=4)
        f = open ('data.txt','w')
        f.write("")
        f.close()
        self.writedoc(json1)
    ...
    def test_new_transaction(self):
        self.blockchain.new_transaction("Diana", "Ana", 100)
        self.assertEqual(len(self.blockchain.current_transactions), 1)
        self.assertEqual(self.blockchain.current_transactions[0].sender, "Diana")
        self.assertEqual(self.blockchain.current_transactions[0].receiver, "Ana")
        ...
        self.assertEqual(len(self.blockchain.blocks), 1)
        json2 = json.dumps(self.blockchain.repr_json(), cls=ComplexEncoder, sort_keys=True, indent=4)
        self.writedoc(json2)
        print(json2)
    ...
    def test_p2p(self):
        ...
        blockchain1.new_transaction("Diana", "Ana", 100)
        blockchain1.new_transaction("Ana", "Diana", 20)
        blockchain1.new_block()

        blockchain2.perform_consensus()
        block1 = json.dumps(blockchain1.repr_json(), cls=ComplexEncoder, sort_keys=True, indent=4)
        block2 = json.dumps(blockchain2.repr_json(), cls=ComplexEncoder, sort_keys=True, indent=4)
        print(block1)
        self.writedoc(block1)
        print(block2)
        self.writedoc(block2)
        import os
        os._exit(os.EX_OK) 
